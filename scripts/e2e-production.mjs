...
// 7. Connecteurs : fail-closed documenté sans COMPOSIO_API_KEY
const apps = await call("GET", "/api/connectors/apps")
check("GET /api/connectors/apps (fail-closed documenté)", apps.status === 200 && apps.json?.configured === false && /COMPOSIO_API_KEY/i.test(JSON.stringify(apps.json)),
  (apps.json?.message ?? "").slice(0, 90))

const conn = await call("POST", "/api/connectors/connections", { toolkitSlug: "github" })
check("POST /api/connectors/connections (503 explicite)", conn.status === 503, (conn.json?.error ?? "").slice(0, 90))
