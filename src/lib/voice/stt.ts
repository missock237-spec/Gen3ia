...
// Internal ZAI ASR shape (undocumented — accessed via unknown cast)
interface ZAIAsrClient {
  transcribe: (opts: { audio: string; language: string }) => Promise<{
    text?: string;
    language?: string;
    confidence?: number;
    duration?: number;
...
async function transcribeZAI(
  ...
  try {
    ...
    // z-ai-web-dev-sdk exposes ASR via an undocumented `audio.asr` path.
    // We use `as unknown as ZAIWithAudio` to avoid unsafe `as any` while
    // still retaining type safety through the ZAIWithAudio interface above.
    const zaiWithAudio = zai as unknown as ZAIWithAudio;
    const asr = zaiWithAudio.audio?.asr;
...
export class SpeechToTextEngine {
  ...
  async transcribe(audioBuffer: Buffer, options: STTOptions = {}): Promise<STTResult> {
    ...
    try {
      const { isFluroHealthy } = await import('@/lib/fluro-client');
      if (await isFluroHealthy()) {
        providers.push({ name: 'fluro', fn: () => transcribeGroq(audioBuffer, options) });
      }
    } catch {
      // Fluro not available
    }
    ...
    for (const provider of providers) {
      try {
        ...
        lastError = error;
        log.warn(`STT provider ${provider.name} failed, trying next`, { error: String(error) });
      }
    }
