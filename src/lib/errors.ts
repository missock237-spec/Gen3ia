...
export const ERROR_CODES = {
  ...
  // ----- Connecteurs d'applications (Composio, ADR-0014) -----
  CONNECTOR_NOT_CONFIGURED: { status: 503, message: "Connecteurs indisponibles : COMPOSIO_API_KEY n'est pas configurée sur le serveur. Ajoutez la clé dans les variables d'environnement pour activer les 1000+ apps.", retryable: false },
  ...
  CONNECTOR_AUTH_FAILED: { status: 402, message: "Clé Composio invalide ou refusée. Vérifiez COMPOSIO_API_KEY.", retryable: false },
  CONNECTOR_UNREACHABLE: { status: 502, message: "Composio injoignable. Vérifiez COMPOSIO_BASE_URL et réessayez.", retryable: true },
  CONNECTOR_ACTION_FAILED: { status: 502, message: "L'exécution de l'action dans l'application externe a échoué.", retryable: true },
  CONNECTOR_RATE_LIMITED: { status: 429, message: "Limite de débit Composio atteinte. Réessayez dans un instant.", retryable: true },
} as const
