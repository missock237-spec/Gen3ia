...
export async function verifyChain(limit?: number): Promise<{ valid: boolean; brokenAt?: string; totalEntries: number }> {
  const entries = await db.immutableAuditLog.findMany({
    ...
    take: limit,
    select: { id: true, entryHash: true, prevHash: true, action: true, entityType: true, entityId: true, detail: true, userId: true, ip: true, createdAt: true },
  })
  ...
  for (const entry of entries) {
    ...
    // Reconstitue exactement le contenu signé à l'écriture :
    // detail était haché comme OBJET (params.detail), il est stocké en JSON.
