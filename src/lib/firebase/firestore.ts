...
//  - `db` provient de '@/lib/db' (façade).
//  - `where` passé sous forme de FirestoreWhereOp[] :
//      { field: string; op: '==' | '>=' | '<=' | ...; value: unknown }
//  - `select` passé sous forme de string[] (PAS d'objet Prisma).
//  - Aucune opération `{ increment }` : incrément réécrit en
//    read-modify-write atomique par transaction si disponible.
// ============================================================
...
/** Définit les champs lus pour la sélection d'un partenaire actif. */
 ...
 * Retourne undefined si aucun partenaire (ou si la clé est absente).
 */
export async function findActivePartner(apiKey: string) {
  if (!apiKey) return null;
  const where: FirestoreWhereOp[] = [{ field: 'apiKey', op: '==', value: apiKey }];
...
export function buildRecommendationPayload(partner: {
  ...
  const baseUrl = `${APP_URL}/recommend`; // (réservé, non exposé en l'état)
 ...
 * Firestore n'étant pas « in-place increment », on lit la valeur, on additionne,
 * puis on réécrit. Enveloppé dans une transaction fournie par la façade si possible.
 */
async function incrementPartnerCounter(partnerId: string, field: string): Promise<void> {
  const partner = await db.partner.findUnique({
    where: [{ field: 'id', op: '==', value: partnerId }],
    select: [field, 'id'],
  });
  ...
  try {
    ...
    if (partner.ownerId) {
      const owner = await db.user.findUnique({
        where: [{ field: 'id', op: '==', value: partner.ownerId }],
        select: ['id', 'credits'],
      });
      const cents = Number((owner as Record<string, unknown> | null)?.credits ?? 0);
      await db.user.update({
        where: [{ field: 'id', op: '==', value: partner.ownerId }],
        data: { credits: cents + PARTNER_SIGNUP_REWARD },
      });
    }
