...
export function checkLoginAttempts(identifier: string): {
  ...
  if (entry.lockedUntil && now < entry.lockedUntil) {
    logger.warn('Tentative de connexion bloquée', { identifier, count: entry.count });
...
export function recordLoginAttempt(identifier: string, success: boolean, ip?: string): void {
  const now = Date.now();
  ...
  if (!success) {
    ...
    if (entry.count >= AUTH_CONFIG.MAX_ATTEMPTS) {
      ...
      logger.error('🔒 COMPTE BLOQUÉ', { identifier, attempts: entry.count, ip });
      prisma.monitoringEvent.create({
        data: {
          userId: identifier,
          eventType: 'security.brute_force',
          source: 'auth',
          message: `Compte bloqué après ${entry.count} tentatives`,
      ...
      logger.warn('Tentatives échouées multiples', { identifier, attempts: entry.count, ip });
    }
  } else {
    attemptsMap.delete(identifier);
...
export async function slowDown(identifier: string): Promise<void> {
  const entry = attemptsMap.get(identifier);
...
export function validatePasswordStrength(password: string): { valid: boolean; score: number; errors: string[] } {
  const errors: string[] = [];
  let score = 0;
  if (password.length >= 8) score += 20; else errors.push('Minimum 8 caractères');
  ...
  if (/[^A-Za-z0-9]/.test(password)) score += 20; else errors.push('Doit contenir un caractère spécial');
