...
export interface PlanLimits { maxAgents: number; maxConcurrent: number; maxTokensPerDay: number; }
const PLAN_LIMITS: Record<string, PlanLimits> = {
  free: { maxAgents: 3, maxConcurrent: 1, maxTokensPerDay: 50000 },
  pro: { maxAgents: 20, maxConcurrent: 5, maxTokensPerDay: 500000 },
};
export function getPlanLimits(plan: string): PlanLimits { return PLAN_LIMITS[plan] || PLAN_LIMITS.free; }
...
export async function getDailyTokenUsage(userId: string): Promise<number> {
  ...
  const result = await db.aiCost.aggregate({ where: { userId, createdAt: { gte: startOfDay } }, _sum: { totalTokens: true } });
...
export async function validateAgentAction(userId: string, plan: string, action: AgentAction, isMultiAgent = false) {
  switch (action) {
    case 'activate': { const c = await checkConcurrentAgents(userId, plan, isMultiAgent); if (!c.allowed) return { allowed: false, reason: `Limite (${c.current}/${c.limit})` }; return { allowed: true }; }
    case 'create': { const c = await checkAgentLimit(userId, plan); if (!c.allowed) return { allowed: false, reason: `Limite (${c.current}/${c.limit})` }; return { allowed: true }; }
    case 'chat': { const c = await checkTokenLimit(userId, plan); if (!c.allowed) return { allowed: false, reason: `Limite (${c.current}/${c.limit})` }; return { allowed: true }; }
