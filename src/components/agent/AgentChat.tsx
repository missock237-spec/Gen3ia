...
const SUGGESTIONS: string[] = [
  'Cherche les dernieres actualites tech',
  'Envoie un email a john@email.com',
  'Va sur google.com',
  'Cree un evenement dans mon calendrier demain',
...
export default function AgentChat({ userId = 'user_' + Date.now() }: AgentChatProps) {
  const [messages, setMessages] = useState<Message[]>([
    { id: '0', text: '🧠 **Agent Genova** active ! Je peux agir sur le web et tes comptes personnels pour toi.\n\nDis-moi ce que tu veux que je fasse.', isUser: false, agent: true, timestamp: new Date() },
  ]);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (data && data.error) {
        displayText = 'Erreur: ' + data.error;
      } else if (data && data.requiresAuth) {
        needsAuth = true;
        displayPlatform = data.platform || '';
        oauthLink = data.oauthUrl || '';
        displayText = 'Connexion requise pour ' + displayPlatform;
      } else if (data && data.requiresApproval) {
        needsApproval = true;
        apprId = data.approvalId || '';
        displayAction = data.action || '';
        displayPlatform = data.platform || '';
        displayText = 'Action risquee sur ' + displayPlatform;
      } else if (data) {
        try {
          displayText = JSON.stringify(data, null, 2).substring(0, 400);
        } catch {
          displayText = 'Action executee avec succes';
        }
      } else {
        displayText = 'Reponse vide';
      }
      ...
      setMessages(prev => [...prev, { id: 'err_' + Date.now(), text: 'Erreur reseau: ' + (e.message || 'inconnue'), isUser: false, agent: true, timestamp: new Date() }]);
  ...
  const handleApprove = async (approvalId: string, approved: boolean) => {
    ...
    try {
      ...
      const data = await response.json();
      setMessages(prev => [...prev, { id: 'app_' + Date.now(), text: approved ? 'Approuve' : 'Rejete', isUser: false, agent: true, timestamp: new Date() }]);
      if (data && data.status === 'completed') {
        setMessages(prev => [...prev, { id: 'res2_' + Date.now(), text: 'Action executee !', isUser: false, agent: true, timestamp: new Date() }]);
      }
    } catch {
      // ignore
    }
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 rounded-2xl overflow-hidden border border-gray-200">
            ...
            <div className={'max-w-[85%] rounded-2xl p-4 ' + (msg.isUser ? 'bg-indigo-500 text-white' : msg.agent ? 'bg-white shadow-sm border border-gray-100' : 'bg-gray-100')}>
              ...
              {msg.requiresAuth && msg.oauthUrl && (
                <a href={msg.oauthUrl} target="_blank" rel="noopener noreferrer" className="mt-3 inline-block bg-indigo-500 text-white px-4 py-2 rounded-xl text-sm font-medium hover:bg-indigo-600 transition">
                  Autoriser l acces a {msg.platform || 'la plateforme'}
                </a>
              )}
      ...
      <div className="px-4 py-2 border-t border-gray-100 flex gap-2 overflow-x-auto">
        {Array.isArray(SUGGESTIONS) && SUGGESTIONS.slice(0, 3).map((s: string, i: number) => (
