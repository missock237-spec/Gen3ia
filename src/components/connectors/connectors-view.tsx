  ...
  const add = async () => {
    ...
    try {
      await apiFetch('/api/connectors', {
        method: 'POST',
        body: JSON.stringify({ name: n, service: s, keyValue: k }),
      });
      setN('');setS('');setK('');setShow(false);
      await load();
    } catch {}
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold">Connecteurs</h1><p className="text-muted-foreground">Cles API et services externes</p></div>
      ...
      {show && <div className="bg-card rounded-xl border p-4 space-y-3">
        ...
        <div className="flex gap-2"><button onClick={add} className="px-4 py-2 bg-primary text-primary-foreground rounded-lg text-sm">Enregistrer</button><button onClick={()=>setShow(false)} className="px-4 py-2 border rounded-lg text-sm">Annuler</button></div>
      </div>}
      {c.length === 0 ? <div className="text-center py-16"><Plug className="h-12 w-12 mx-auto mb-4" /><h3>Aucun connecteur</h3><p className="text-sm text-muted-foreground mt-2">Ajoutez vos cles API pour connecter des services externes</p></div>
        : c.map((x:any) => <div key={x.id} className="bg-card rounded-xl border p-4 flex items-center gap-4">
          <Key className="h-5 w-5 text-primary" />
          <div className="flex-1"><h3>{x.name}</h3><p className="text-xs text-muted-foreground">{x.service}</p></div>
