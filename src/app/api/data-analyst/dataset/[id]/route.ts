...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const dataset = await prisma.dataset.findFirst({ where: { id: (await params).id, userId: auth.userId } });
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const existing = await prisma.dataset.findFirst({ where: { id: (await params).id, userId: auth.userId } });
    if (!existing) return NextResponse.json({ error: 'Dataset introuvable' }, { status: 404 });
    const updated = await prisma.dataset.update({ where: { id: (await params).id }, data: {
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const dataset = await prisma.dataset.findFirst({ where: { id: (await params).id, userId: auth.userId } });
    if (!dataset) return NextResponse.json({ error: 'Dataset introuvable' }, { status: 404 });
    await prisma.dataset.delete({ where: { id: (await params).id } });
