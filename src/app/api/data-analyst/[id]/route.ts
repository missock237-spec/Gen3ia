...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const dashboard = await prisma.dashboard.findFirst({ where: { id: (await params).id, userId: auth.userId } });
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const existing = await prisma.dashboard.findFirst({ where: { id: (await params).id, userId: auth.userId } });
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const dashboard = await prisma.dashboard.findFirst({ where: { id: (await params).id, userId: auth.userId } });
    if (!dashboard) return NextResponse.json({ error: 'Dashboard introuvable' }, { status: 404 });
    await prisma.dashboard.delete({ where: { id: (await params).id } });
