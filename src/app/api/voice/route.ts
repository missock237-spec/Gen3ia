...
export async function GET(r: NextRequest) {
  try {
    ...
    const s = await db.voiceSession.findMany({ where: { userId: d.userId }, orderBy: { createdAt: 'desc' }, take: 20 });
...
export async function POST(r: NextRequest) {
  try {
    ...
    const { userId, type, language } = b;
    if (!userId || !type) return NextResponse.json({ error: 'userId et type requis' }, { status: 400 });
    const s = await db.voiceSession.create({ data: { userId, type, status: 'active', language } });
