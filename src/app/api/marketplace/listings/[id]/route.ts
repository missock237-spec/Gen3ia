...
import {
  ...
  updateListing,
  deleteListing,
} from '@/lib/marketplace/listing-manager'
...
export async function GET(
  ...
  try {
    ...
    const listing = await getListingForViewer(auth.userId, id)
...
export async function PUT(
  ...
  try {
    ...
    const status =
      message.includes('not found') || message.includes('not authorized') ? 404 : 400
