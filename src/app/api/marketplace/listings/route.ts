...
import type { ListingType, ListingCategory } from '@/lib/marketplace/listing-manager'
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Validate and narrow to typed values — no `as any` needed
    ...
    const minPrice =
      minPriceRaw !== null && minPriceRaw !== '' ? Number(minPriceRaw) : undefined
    const maxPrice =
      maxPriceRaw !== null && maxPriceRaw !== '' ? Number(maxPriceRaw) : undefined
    ...
    log.error('GET /api/marketplace/listings failed', { error })
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!type || !name || !description) {
      return secureResponse(
        NextResponse.json({ error: 'Type, name, and description are required' }, { status: 400 }),
        request
      )
    }
    ...
    if (!VALID_TYPES.includes(type)) {
      return secureResponse(
        NextResponse.json({ error: `Invalid type. Allowed: ${VALID_TYPES.join(', ')}` }, { status: 400 }),
        request
      )
    }
    ...
    if (category !== undefined && !VALID_CATEGORIES.includes(category)) {
      return secureResponse(
        NextResponse.json({ error: `Invalid category. Allowed: ${VALID_CATEGORIES.join(', ')}` }, { status: 400 }),
        request
      )
    }
    ...
    if (typeof description !== 'string' || description.trim().length < 10 || description.length > 5000) {
      return secureResponse(
        NextResponse.json({ error: 'Description must be between 10 and 5000 characters' }, { status: 400 }),
        request
      )
    }
    ...
    if (tags !== undefined) {
      if (!Array.isArray(tags) || tags.length > 20) {
        return secureResponse(
          NextResponse.json({ error: 'Tags must be an array with at most 20 items' }, { status: 400 }),
          request
        )
      }
      const invalidTag = tags.find((tag: unknown) => typeof tag !== 'string' || (tag as string).length > 40)
      if (invalidTag !== undefined) {
        return secureResponse(
          NextResponse.json({ error: 'Each tag must be a string of at most 40 characters' }, { status: 400 }),
          request
        )
      }
    }

    if (price !== undefined && (!Number.isFinite(price) || typeof price !== 'number' || price < 0)) {
      return secureResponse(
        NextResponse.json({ error: 'Price must be a number greater than or equal to 0' }, { status: 400 }),
        request
      )
    }
    ...
    if (previewUrl !== undefined && previewUrl !== null && previewUrl !== '') {
      if (typeof previewUrl !== 'string' || !isValidUrl(previewUrl)) {
        return secureResponse(
          NextResponse.json({ error: 'previewUrl must be a valid URL' }, { status: 400 }),
          request
        )
      }
    }
    ...
    if (config !== undefined && !isPlainObject(config)) {
      return secureResponse(
        NextResponse.json({ error: 'config must be a JSON object' }, { status: 400 }),
        request
      )
    }
    ...
    if (metadata !== undefined && !isPlainObject(metadata)) {
      return secureResponse(
        NextResponse.json({ error: 'metadata must be a JSON object' }, { status: 400 }),
        request
      )
    }
    ...
    log.error('POST /api/marketplace/listings failed', { err })
    const message = err instanceof Error ? err.message : 'Failed to create listing'
    const status =
      message.includes('Invalid') || message.includes('must be') || message.includes('required')
        ? 400
        : 500
