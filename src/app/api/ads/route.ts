 ...
 * Returns an eligible ad for the requested placement
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return secureResponse(
      NextResponse.json(
        { error: 'Failed to fetch ad', details: err instanceof Error ? err.message : 'Unknown error' },
        { status: 500 }
      ),
      request
    );
...
/**
 * POST /api/ads
 * Body: { adUnitId: string }
 * Records an ad view and awards credits
 */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!adUnitId) {
      return secureResponse(
        NextResponse.json({ error: 'Missing required field: adUnitId' }, { status: 400 }),
        request
      );
    }
    ...
    if (!adUnit) {
      return secureResponse(
        NextResponse.json({ error: 'Invalid ad unit' }, { status: 400 }),
        request
      );
    }
    ...
    if (adUnit.status !== 'active') {
      return secureResponse(
        NextResponse.json({ error: 'This ad is no longer active' }, { status: 400 }),
        request
      );
    }
    ...
    if (!result.success) {
      return secureResponse(
        NextResponse.json(result, { status: 429 }),
        request
      );
    }
    ...
    return secureResponse(
      NextResponse.json(
        { error: 'Failed to process ad reward', details: err instanceof Error ? err.message : 'Unknown error' },
        { status: 500 }
      ),
      request
    );
