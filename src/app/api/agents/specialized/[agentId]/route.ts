...
export async function GET(request: NextRequest, { params }: { params: Promise<{ agentId: string }> }) {
  ...
  try {
    const agent = await prisma.agent.findFirst({
      where: { id: (await params).agentId, OR: [{ ownerId: auth.userId }, { isPublic: true }] },
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ agentId: string }> }) {
  ...
  try {
    ...
    const agent = await agentSpecialization.updateAgent((await params).agentId, auth.userId, body);
    return NextResponse.json({ success: true, agent });
  } catch (err) {
    return NextResponse.json({ error: String(err) }, { status: 500 });
  }
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ agentId: string }> }) {
  ...
  if (error || !auth) return NextResponse.json({ error: 'Non authentifie' }, { status: 401 });
  try {
    const agent = await prisma.agent.findFirst({ where: { id: (await params).agentId, ownerId: auth.userId } });
    if (!agent) return NextResponse.json({ error: 'Agent introuvable' }, { status: 404 });
    await prisma.agent.delete({ where: { id: (await params).agentId } });
