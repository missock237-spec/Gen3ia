...
import { validateWebhook, createSecurePayload } from '@/lib/webhook-security';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'test' && body.webhookId) {
      ...
      try {
        const { payload, signature, headers: securityHeaders } = createSecurePayload(
          'webhook.test',
          { message: 'Test de livraison depuis Gen3ia' },
          secret
        );
        ...
        const res = await fetch(wh.url as string, {
          ...
          headers: {
            ...securityHeaders,
            'X-Gen3ia-Delivery': `test_${Date.now()}`,
        ...
        return NextResponse.json({
          success: res.ok,
          data: { status: res.status, durationMs, body: await res.text().catch(() => '(impossible de lire la reponse)') },
        });
      } catch (err) {
        const durationMs = Date.now() - startTime;
        await db.$executeRawUnsafe(`
          UPDATE webhook_endpoints SET last_error = $1, last_response_status = NULL WHERE id = $2
        `, [err instanceof Error ? err.message : String(err), body.webhookId]);
        return NextResponse.json({ success: false, data: { durationMs, error: err instanceof Error ? err.message : 'Erreur de connexion' } });
      }
    }
    ...
    `, [
      `wh_${randomBytes(12).toString('hex')}`, body.name, body.description || '', body.url,
      secret, JSON.stringify(events), JSON.stringify(body.headers || {}),
      JSON.stringify(body.retryConfig || { maxRetries: 3, backoffMs: 1000 }),
      body.timeoutMs || 10000, body.isActive !== false, userId,
    ]);

    log.info('Webhook created', { userId, webhookName: body.name, events });
    return NextResponse.json({
      success: true, data: { name: body.name, url: body.url, events, secret },
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!webhookId) return NextResponse.json({ success: false, error: 'webhookId requis' }, { status: 400 });

    switch (action) {
      case 'toggle':
        await db.$executeRawUnsafe(`UPDATE webhook_endpoints SET is_active = NOT is_active WHERE id = $1 AND user_id = $2`, [webhookId, userId]);
        return NextResponse.json({ success: true, message: 'Webhook mis a jour' });
      case 'delete':
        await db.$executeRawUnsafe(`DELETE FROM webhook_endpoints WHERE id = $1 AND user_id = $2`, [webhookId, userId]);
      ...
      case 'update':
        ...
        if (updates.length > 0) {
          ...
          await db.$executeRawUnsafe(`UPDATE webhook_endpoints SET ${updates.join(', ')} WHERE id = $${idx} AND user_id = $${idx + 1}`, params);
        }
        return NextResponse.json({ success: true, message: 'Webhook mis a jour' });
      default:
